...
void handle_wcc_message(uint8_t *output_buffer, size_t buffer_size)
{
    ...
    for (unsigned int decoder_index = 0; decoder_index < decoder_amount; ++decoder_index)
    {
        ...
        // Get element amount
        uint8_t element_amount = (uint8_t)output_buffer[element_data_start_ind++];

        for (unsigned int element_index = 0; element_index < element_amount; ++element_index)
        {

            loco_element_settings current_element;

            ESP_LOGI(TAG, "============================");
            ESP_LOGI(TAG, "Loading data for element: %d", element_index);

            // Get Connection Count
            unsigned char connection_count = output_buffer[element_data_start_ind++];
            ESP_LOGI(TAG, "Connection count: %d", connection_count);
            // Move index to element->connection_count

            // Allocate memory for Connections for the current element
            current_element.connections = (Connection *)malloc(connection_count * sizeof(Connection));

            // Iterate over connections
            for (unsigned int connection_index = 0; connection_index < connection_count; ++connection_index)
            {
                ESP_LOGI(TAG, "\nConnection number: %d", connection_index);

                // Allocate memory for Connection
                Connection connection;

                // Get Connection Type
                connection.type = output_buffer[element_data_start_ind];
                ESP_LOGI(TAG, "Connection type: %d", connection.type);
                ++element_data_start_ind;

                // Get Physical GPIO number
                connection.GPIO = output_buffer[element_data_start_ind];
                ESP_LOGI(TAG, "Connected to GPIO %d\n", connection.GPIO);
                ++element_data_start_ind;

                current_element.connections[connection_index] = connection;
            }
            ...
            for (unsigned int state_number = 0; state_number < current_element.state_count; ++state_number)
            {
                ...
                for (unsigned int value_number = 0; value_number < state.value_count; ++value_number)
                {
                    ...
                    ESP_LOGI(TAG, "Value:");
                    ESP_LOG_BUFFER_HEX(TAG, value.val, value.val_length);
                    ESP_LOGI(TAG, "=======================");
